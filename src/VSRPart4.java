...
public class VSRPart4
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			vsr.inverseNewton(true);
			vsr.inverseNewton(false);
		} catch (Exception e)
		{
			e.printStackTrace();
		}
	...
	public void inverseNewton(boolean intermediate) throws InterruptedException
	{
		double a1, a2;
		double targetX,targetY;
		if (intermediate)
		{
			// We need to travel around the obstacle first
		...
		while (error > minError)
		{
			...
			// Gain is proportional to distance from the target.
			double dist = Math.hypot(tracker.x - targetX, tracker.y
					- targetY);
			gain = 1. - (dist / 500);
			if (gain > 1)
				gain = 1;
			...
			// Clamp the motor movements if the movement is too large.
			...
			error = Math.hypot(tracker.x - targetX, tracker.y
					- targetY);
			System.out.println("Error " + error);
			System.out.printf("[%+08f\t%+08f]\n[%+08f\t%+08f]\n",
					mdJac.get(0, 0), mdJac.get(0, 1), mdJac.get(1, 0),
					mdJac.get(1, 1));
