...
public class VSRPart4
{
	...
	public double midTargetX = 0;
	public double midTargetY = 0;
	public double baseX = 0;
	public double baseY = 0;
	public double obstX = 0;
	public double obstY = 0;
	...
	public static void main(String[] args)
	{
		...
		try
		{
			// Go to the midpoint between the base and the obstacle
			vsr.getInitialPosition(true);
			vsr.calculateJacobian(0, 0);
			vsr.inverseNewtonWithInitialGuess(vsr.midTargetX, vsr.midTargetY);
	...
	public VSRPart4()
	{
		...
		{
			System.err
					.println("Couldn't connect to the EV3: " + e.getMessage());
			return;
		}
	...
	public void getInitialPosition(boolean intermediate)
	{
		...
		midTargetX = (obstX + baseX) / 2;
		midTargetY = (obstY + baseY) / 2;
		midTargetX = (obstX + midTargetX) / 2;
		midTargetY = (obstY + midTargetY) / 2;
		
		System.out.printf("Target: %f, %f   Mid: %f, %f   Base: %f, %f   Obst: %f, %f\n",
				targetX,
				targetY,
				midTargetX,
				midTargetY,
		...
		maxLocalPoints = (int) (Math.hypot(targetX - x, targetY - y) / 60) + 1;
	...
	public void inverseNewtonWithInitialGuess(double posX, double posY)
	{
		...
		{
			System.err.println("LOCAL POINT: " + n + " out of "
					+ (int) maxLocalPoints);
		...
		if (tracker.isConnected)
			System.err.println("TARGET REACHED!!! with error: " + (int) error
					+ " pixels, max error allowed: " + minError);
		try
		{
			Thread.sleep(5000);
	...
	public double[] inverseNewton(double posX, double posY, double a1,
			double a2, int iteration)
	{
